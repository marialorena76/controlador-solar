...
async function saveUbicacionEnServidor() {
  const lat = userSelections?.location?.lat;
  const lng = userSelections?.location?.lng;

  const cityString = (userSelections.cityName && userSelections.cityName.trim())
    ? userSelections.cityName.trim()
    : (lat != null && lng != null ? `${lat.toFixed(6)},${lng.toFixed(6)}` : '');

  if (!cityString) {
    console.warn('Sin ciudad para guardar todavía');
    return;
  }
  ...
  const res = await fetch('/api/ubicacion', {
    ...
    body: JSON.stringify({ city: cityString, lat, lng })
  });
  const data = await res.json();
  if (!res.ok) throw new Error(data?.error || 'No se pudo guardar la ciudad en B7');

  localStorage.setItem('ubicacionSeleccionada', JSON.stringify({ city: cityString, lat, lng }));
  console.log('✅ Ubicación guardada en B7:', data);
}
...
async function initRotacionSection() {
    ...
    function updateAngleFieldsVisibilityAndData(selectedText) {
        ...
        const isInclinacionFijaVertical = (cleanSelectedText === "inclinacin fija, rotación sobre un eje vertical");
...
async function initModeloMetodoSection() {
    ...
    if (metodoCalculoSeleccionado === "Cielo Isotrópico") {
        ...
        autoSelectModel = "Mtodo Liu-Jordan";
...
function initMap() {
            ...
            geocoderControlInstance.options.geocoder.reverse(e.latlng, map.options.crs.scale(map.getZoom()), async function(results) {
                ...
                if (r && r.name && locationDisplay) {
                    ...
                    if (ciudadResult) {
                        ...
                        userSelections.cityName = ciudadResult.nombre;
    ...
    geocoderControlInstance = L.Control.geocoder({
        placeholder: 'Ej: Buchardo 3232, Olavarra', // Nuevo placeholder
        errorMessage: 'No se encontr la dirección.',
...
// --- Nueva función para buscar el cdigo de la ciudad (refactorizada) ---
...
async function persistSelectedCityName(nombreCiudad) {
  // Guardamos también en userSelections para tenerlo siempre a mano
  userSelections.cityName = (nombreCiudad || '').trim() || null;

  // Si no tenemos nada, no rompas, pero dejalo logueado
  if (!userSelections.cityName) {
    console.warn('persistSelectedCityName: nombreCiudad vacío');
    return;
  }

  // 1) Intentá usar /api/ubicacion (nuevo endpoint)
  try {
    const res = await fetch('/api/ubicacion', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        city: userSelections.cityName,
        lat: userSelections?.location?.lat,
        lng: userSelections?.location?.lng
      })
    });
    const data = await res.json();
    if (!res.ok) throw new Error(data?.error || 'Fallo /api/ubicacion');
    console.log('✅ B7 actualizado vía /api/ubicacion:', data);
    ...
    console.warn('⚠️ /api/ubicacion no disponible, pruebo /api/excel/update_cell', e);
  }

  // 2) Fallback: usar el endpoint que ya tenías (/api/excel/update_cell)
  try {
    const res2 = await fetch('/api/excel/update_cell', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ ciudad: userSelections.cityName })
    });
    const data2 = await res2.json();
    if (!data2?.success) throw new Error(data2?.error || 'Fallo /api/excel/update_cell');
    console.log('✅ B7 actualizado vía /api/excel/update_cell:', data2);
  } catch (e2) {
    console.error('❌ No pude actualizar B7 en el servidor:', e2);
    throw e2;
  }
}
...
function setupNavigationButtons() {
    ...
    if (nextFromSuperficieButton) {
        nextFromSuperficieButton.addEventListener('click', () => {
            ...
            if (userSelections.superficieRodea.valor === null) {
                alert("Por favor, seleccione una opcin de superficie.");
                return;
            }
    ...
    document.getElementById('next-to-paneles-from-rotacion')?.addEventListener('click', () => {
        ...
        //     alert("Por favor, seleccione una opción de rotacin.");
        //     return;
        // }
    ...
    if (finalizarCalculoBtn) {
        finalizarCalculoBtn.addEventListener('click', async (event) => {
            event.preventDefault();
            const cont = document.getElementById('resultados-informe');
            if (cont) cont.innerHTML = '<b>Generando informe...</b>';

            console.log('Finalizar Cálculo clickeado. Enviando datos al backend para generar informe...');

            
                // 1) Asegurá que B7 quede grabado (usa ciudad del flujo nuevo o legacy)
    try {
      const nombreCiudad =
        (userSelections?.ciudad && userSelections.ciudad.nombre) ||
        userSelections?.cityName || '';
      await persistSelectedCityName(nombreCiudad);
    } catch (e) {
      console.warn('No se pudo persistir B7 antes del informe:', e);
    }

    // 2) Pedir informe (Resultados!B3:L50)
    const response = await fetch('/api/informe');
    const data = await response.json();
    if (!response.ok) throw new Error(data?.error || 'No se pudo leer el informe');

    // Guardado básico por si querés reutilizar luego
    localStorage.setItem('informeSolarResultados', JSON.stringify(data.resultados));

    // 3) Determinar tipo de usuario como antes (pero sin depender del backend)
    const selectedUserType = typeof userSelections.userType === 'string'
      ? userSelections.userType.toLowerCase()
      : null;
    const isBasicUser = selectedUserType === 'basico';

    if (isBasicUser) {
      // Render simple de tabla en la misma pantalla
      ...
      if (resultadosContainer) {
        ...
        const filas = (data.resultados || []).map(
          fila => `<tr>${fila.map(c => `<td>${c}</td>`).join('')}</tr>`
        ).join('');

        const tableWrapper = document.createElement('div');
        tableWrapper.className = 'basic-report-table-wrapper';
        tableWrapper.innerHTML = `
          <h2>Informe (Resultados!B3:L50)</h2>
          <div style="overflow:auto;">
            <table border="1" cellpadding="6" cellspacing="0">${filas}</table>
          </div>
          <p style="margin-top:8px;opacity:.75;">*Datos leídos directamente del Excel del servidor.</p>
        `;
        ...
        // Si no hay contenedor, vamos a informe.html
        window.location.href = 'informe.html';
      }
    } else {
      // Expert: igual que antes, a informe.html
      window.location.href = 'informe.html';
    }
...
document.addEventListener('DOMContentLoaded', async () => {
...
}); // Inicializa el mapa con la ubicación por defecto.
    ...
    // ********************************************************************************
    // MANTENIENDO TU CDIGO ORIGINAL DESPUÉS DEL DOMContentLoaded:
    ...
    // loadCharts(); // Si tenías una funcin para cargar gráficos
