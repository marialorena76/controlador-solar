...
# --- Paths to new static JSON option files ---
SUPERFICIE_OPTIONS_JSON_PATH = os.path.join(PROJECT_ROOT, 'public', 'superficie_options.json')
RUGOSIDAD_OPTIONS_JSON_PATH = os.path.join(PROJECT_ROOT, 'public', 'rugosidad_options.json')
ROTACION_OPTIONS_JSON_PATH = os.path.join(PROJECT_ROOT, 'public', 'rotacion_options.json')


def get_options_from_json(file_path, option_name):
    """
    Helper function to read options from a JSON file.
    Includes robust error handling.
    """
    try:
        print(f"DEBUG: Solicitud de opciones para '{option_name}'. Leyendo desde: {file_path}")
        if not os.path.exists(file_path):
            print(f"ERROR CRITICO: El archivo de opciones {file_path} no fue encontrado.")
            return jsonify({"error": f"Archivo de datos de '{option_name}' no encontrado en el servidor."}), 404

        with open(file_path, 'r', encoding='utf-8') as f:
            data = json.load(f)

        print(f"DEBUG: Se leyeron {len(data)} opciones para '{option_name}'.")
        return jsonify(data)

    except json.JSONDecodeError as je:
        print(f"ERROR en get_options_from_json para '{option_name}': Error al decodificar {file_path}: {je}")
        return jsonify({"error": f"El archivo de datos de '{option_name}' está corrupto."}), 500
    except Exception as e:
        import traceback
        print(f"ERROR GENERAL en get_options_from_json para '{option_name}': {e}")
        print(traceback.format_exc())
        return jsonify({"error": f"Error interno del servidor al obtener opciones de '{option_name}': {str(e)}"}), 500


# --- REFACTORED ROUTES to use static JSON files ---

@app.route('/api/superficie_options', methods=['GET'])
def get_superficie_options():
    return get_options_from_json(SUPERFICIE_OPTIONS_JSON_PATH, "superficie")

@app.route('/api/rugosidad_options', methods=['GET'])
def get_rugosidad_options():
    return get_options_from_json(RUGOSIDAD_OPTIONS_JSON_PATH, "rugosidad")

@app.route('/api/rotacion_options', methods=['GET'])
def get_rotacion_options():
    return get_options_from_json(ROTACION_OPTIONS_JSON_PATH, "rotacion")
